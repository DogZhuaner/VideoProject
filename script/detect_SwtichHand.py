            ...
            if hand_detected:
                ...
                if (
                    ...
                    try:
                        ...
                        # 进入下一步切图并且识别
                        split_image_by_regions(Global_Config.live_capture_path, Global_Config.region_json_path,
                                               Global_Config.split_path)

                        # 使用yolo检测触点
                        detect_and_save(Global_Config.split_path, Global_Config.predict_txt_path, Global_Config.contact,
                                        ["empty", "wired", "wired2"])

                        # 合并并处理数据
                        merge_txt_files(Global_Config.label_6area_path, Global_Config.predict_txt_path,
                                        Global_Config.result_csv_path)

                        # 更新新添加的触点对,validate_and_move_files()返回的是一对触点，list列表格式
                        update_connected_components(*validate_and_move_files())


                        match_subgraphs()
                        matches = match_subgraphs()
                        for match in matches:
                            print(f"匹配到连通图，序号: {match['匹配序号']}, 分值: {match['分值']}")
